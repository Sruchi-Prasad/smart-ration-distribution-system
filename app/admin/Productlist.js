...
export default function AdminManageProducts() {
    ...
    const renderItem = ({ item }) => (
        <View style={styles.card}>
            <View>
                ...
                <Text>Price: ₹{item.price} / {item.unit}</Text>
                <Text>Stock: {item.quantity} {item.unit}</Text>
                <Text>Min Stock Alert: {item.minStock}</Text>
            </View>
    ...
    return (
        <View style={styles.container}>
            <Text style={styles.title}>Admin Manage Products</Text>
            ...
            <Modal visible={modalVisible} animationType="slide">
                <View style={styles.modal}>
                    ...
                    <TextInput placeholder="Product Name" style={styles.input} value={form.name}
                        onChangeText={(t) => setForm({ ...form, name: t })} />
                    <TextInput placeholder="Category" style={styles.input} value={form.category}
                        onChangeText={(t) => setForm({ ...form, category: t })} />
                    <TextInput placeholder="Price" keyboardType="numeric" style={styles.input} value={form.price}
                    ...
                    <TextInput placeholder="Unit (kg, litre, etc.)" style={styles.input} value={form.unit}
                        onChangeText={(t) => setForm({ ...form, unit: t })} />
                    <TextInput placeholder="Minimum Stock Alert" keyboardType="numeric" style={styles.input} value={form.minStock}
...
const styles = StyleSheet.create({
    ...
    card: {
        backgroundColor: "white", padding: 15, borderRadius: 10, marginBottom: 12,
        flexDirection: "row", justifyContent: "space-between", elevation: 2
    },
    name: { fontSize: 16, fontWeight: "bold" },
    actions: { flexDirection: "row", gap: 15 },
    addBtn: {
        position: "absolute", right: 20, bottom: 20, backgroundColor: "#1E88E5",
    ...
    emptySubText: { fontSize: 14, color: "#00796B", marginTop: 5, fontWeight: "600" }
});